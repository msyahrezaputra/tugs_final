...
class Rokok extends REST_Controller {
    function __construct(){
        parent::__construct();
        $this->load->model('Rokok_model');
    }

    public function index_get(){
        $id = $this->get('id');
        if($id != ''){
            $data_mhs = $this->Rokok_model->getRokok($id);
        }else{
            $data_mhs = $this->Rokok_model->getRokok();
        }
        if($data_mhs == true){
          $this->response(array(
            'status'  => 'true',
    ...
    public function tambah_post(){
        if($this->Rokok_model->tambahDataRokok($_POST) > 0) {
          $this->response(array(
            'status'  => 'true',
            'code'    => '200',
    ...
    public function ubah_post(){
      if($this->Rokok_model->ubahDataRokok($_POST) > 0) {
        $this->response(array(
          'status'  => 'true',
          'code'    => '200',
    ...
    public function hapus_get($id){
      if($this->Rokok_model->hapusDataRokok($id)) {
         $this->response(array(
          'status'  => 'true',
          'code'    => '200',
